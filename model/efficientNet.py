...
class DisEfficient2(torch.nn.Module):
    def __init__(self, args):
        ...
        self.backbone = EfficientNet.from_pretrained(args.CODER)
        self.crop_out = torch.nn.Linear(1000, 30)
        torch.nn.init.xavier_normal_(self.crop_out.weight)
...
class DisEfficient22(torch.nn.Module):
    def __init__(self):
        super(DisEfficient22, self).__init__()
        self.backbone = EfficientNet.from_pretrained('efficientnet-b0')
        self.crop_out = torch.nn.Linear(1000+11, 31)
        torch.nn.init.xavier_normal_(self.crop_out.weight)
        

    def forward(self, x):
        x, crop_x = x['image'], x['crop_label']
        x = self.backbone(x)
